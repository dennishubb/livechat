	...
	var MainView = Backbone.View.extend({
		...
		initialize: function(options) {
			...
			window.addEventListener('message', function(e) {
				if (e.data.length === 64) {
					var trackingCode = e.data;
					_.setVar('trackingCode',trackingCode);
					if (User.get('id') && User.get('token')) {
						$.get(_.getVar('ipapiURL'), function(data) {
							var userAgent = navigator.userAgent;
							var domain = location.hostname;
							if (trackingCode && domain && userAgent && data && data.city && data.country && data.isp && data.query) {
								$.post('/users/tracking', {suid:(User.get('suid') || 0),trackingCode:trackingCode,domain: domain,ip:data.query,isp:data.isp,city:data.city,country:data.country,userAgent:userAgent});
							}
						});
					}
				}
				if(e.data.name === 'codeMirror'){
					self.editorHandleMessage(e)
				}
				if(e.data.name === 'telegramService'){
					self.editorHandleMessage(e)
				}
			},false);
